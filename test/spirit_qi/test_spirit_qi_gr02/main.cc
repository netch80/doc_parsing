...
namespace {
  ...
  struct Parser : qi::grammar<SI, double(), ascii::space_type> {
    ...
    Parser() : Parser::base_type(expression) {
      ...
      // NB simpler approaches don't work, so, use two semantic actions
      // and lambda with getting ref to "exposed".
      power = (atom [ _val = _1 ] >> lit("**") >> power [
                ([](double& attribute, f_context_d1& context, bool& pass) {
                  double& exposed = boost::fusion::at_c<0>(context.attributes);
                  exposed = pow(exposed, attribute);
                  pass = true;
                })
            ])
          | atom [ _val = _1 ];
...
void testExpressionFail(const char *expr_text) {
  if (verbose) {
    printf("testExpressionFail: expr=<%s>\n", expr_text);
  }
  double result = -1.0e40;
  bool r = parseExpression(expr_text, /*ref*/ result);
  if (r) {
    throw std::runtime_error((boost::format("parsing not failed; text=<%1%>") % expr_text).str());
  }
}
